...
from util.metrics import loss_c, loss_fine_poi
...
class FViT(nn.Module):
    ...
    def __init__(self, img_size=8, patch_size=2, in_chans=32, sample_index=None, fmargin=20,
                 embed_dim=1024, depth=24, num_heads=16, random=random, type='softmax',
                 decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16, upscale_factor=4,
        ...
        if upscale_factor == 4:
            ...
            self.poi = np.load("data/POI_16.npy")
        self.poi = torch.from_numpy(self.poi).cuda().float()
        ...
        # self.pos_embed3 = nn.Parameter(torch.zeros(1, int(num_patches / self.r) + 1, embed_dim),
        #                                requires_grad=False)  # sapce
        ...
        self.encoder_decoder = nn.Linear(32, embed_dim, bias=True)
        self.decoder_embed = nn.Linear(embed_dim, decoder_embed_dim, bias=True)

        self.mask_token = nn.Parameter(torch.zeros(1, 1, decoder_embed_dim))

        self.decoder_pos_embed = nn.Parameter(torch.zeros(1, self.r + 1, decoder_embed_dim),
                                              requires_grad=False)  # fixed sin-cos embedding
        ...
        for i in range(num_patches):
            self.Decoder_Blocks.append(self.decoder_blocks)
        ...
        # --------------------------------------------------------------------------
        # M2-Normalization
        self.rr = int(math.sqrt(self.r))      # 4
        self.den_softmax = N2_Normalization(self.rr)
        self.recover = Recover_from_density(self.rr)
    ...
    def random_masking(self, x, mask_ratio):
        ...
        self.pos_embed2[:, 1:, :] = torch.gather(self.pos_embed.data, dim=1,
                                                 index=ids_keep.add(1).unsqueeze(-1).repeat(1, 1, x.shape[2])[
                                                     0].unsqueeze(0))

        # self.pos_embed3[:, :1, :] = self.pos_embed[:, :1, :]
        # self.pos_embed3[:, 1:, :] = torch.gather(self.pos_embed.data, dim=1,
        #                                          index=self.id_s_r_er_keep.add(1).unsqueeze(-1).unsqueeze(0).repeat(1,
        #                                                                                                             1,
        #                                                                                                             x.shape[
        #                                                                                                                 2]))
    ...
    def forward_decoder(self, x):
        ...
        Xs = torch.zeros(N, LL, D).cuda()
        for i in range(L):
            mask_tokens = self.mask_token.repeat(N, self.r, 1)
            x_ = torch.cat([x[:, i, :].unsqueeze(1), mask_tokens], dim=1)  # no cls token
            ...
            for j in range(self.r):
                j_x = int(j // self.rr)
                j_y = int(j % self.rr)
                all_x = i_x * self.rr + j_x
                all_y = i_y * self.rr + j_y
                all = all_x * (n * self.rr) + all_y

                Xs[:, all, :] = x_[:, j + 1, :]


        x = self.decoder_norm(Xs)
    ...
    def forward_poiloss(self, latent):
        poi = self.fpoi_embed(self.poi).squeeze(-1)
        # loss = loss_fine_poi(latent, poi, self.fmargin, self.type)
    ...
    def forward(self, imgs, latent, target, mask_ratio=0.75):
        # N, C, H, W = HX.shape
        ...
        B, L, D = latent.shape
        latent = rearrange(latent, 'b (w h) d -> b d w h', b=B, w=int(math.sqrt(L)))

        latent = self.patch_embed(latent)

        # ids_restore = self.random_masking(latent, mask_ratio)

        latent = self.encoder_decoder(latent)
        # add pos embed w/o cls token
        # latent = latent + self.pos_embed2[:, 1:, :]
        ...
        return loss, poi_loss, poi_loss, pred
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=32, patch_size=4, in_chans=32, embed_dim=64):
        super().__init__()
...
def mae_vit_1(patch_size, in_chans, **kwargs):
    model = FViT(
        patch_size=patch_size, embed_dim=32, depth=4, num_heads=4, in_chans=in_chans,
...
def mae_vit_2(patch_size, in_chans, **kwargs):
    model = FViT(
        patch_size=patch_size, embed_dim=32, depth=6, num_heads=8, in_chans=in_chans,
        decoder_embed_dim=64, decoder_depth=6, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
...
def mae_vit_4(patch_size, in_chans, **kwargs):
    model = FViT(
        patch_size=patch_size, embed_dim=32, depth=10, num_heads=8, in_chans=in_chans,
        decoder_embed_dim=128, decoder_depth=6, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
...
def mae_vit_5(patch_size, in_chans, **kwargs):
    model = FViT(
        patch_size=patch_size, embed_dim=32, depth=12, num_heads=12, in_chans=in_chans,
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def mae_vit_6(patch_size, in_chans, **kwargs):
    model = FViT(
        patch_size=patch_size, embed_dim=32, depth=12, num_heads=12, in_chans=in_chans,
...
def mae_vit_7(patch_size, in_chans, **kwargs):
    model = FViT(
        patch_size=patch_size, embed_dim=32, depth=12, num_heads=12, in_chans=in_chans,
...
def mae_vit_8(patch_size, in_chans, **kwargs):
    model = FViT(
        patch_size=patch_size, embed_dim=32, depth=12, num_heads=12, in_chans=in_chans,
        decoder_embed_dim=64, decoder_depth=12, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def mae_vit_9(patch_size, in_chans, **kwargs):
    model = FViT(
        patch_size=patch_size, embed_dim=32, depth=12, num_heads=12, in_chans=in_chans,
        decoder_embed_dim=64, decoder_depth=16, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def mae_vit_10(patch_size, in_chans, **kwargs):
    model = FViT(
        patch_size=patch_size, embed_dim=32, depth=12, num_heads=12, in_chans=in_chans,
        decoder_embed_dim=128, decoder_depth=16, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def mae_vit_11(patch_size, in_chans, **kwargs):
    model = FViT(
        patch_size=patch_size, embed_dim=32, depth=12, num_heads=12, in_chans=in_chans,
        decoder_embed_dim=64, decoder_depth=20, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def mae_vit_12(patch_size, in_chans, **kwargs):
    model = FViT(
        patch_size=patch_size, embed_dim=32, depth=1, num_heads=1, in_chans=in_chans,
        decoder_embed_dim=96, decoder_depth=12, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def mae_vit_13(patch_size, in_chans, **kwargs):
    model = FViT(
        patch_size=patch_size, embed_dim=32, depth=1, num_heads=1, in_chans=in_chans,
        decoder_embed_dim=96, decoder_depth=12, decoder_num_heads=8, norm_pix_loss=True,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def mae_vit_14(patch_size, in_chans, **kwargs):
    model = FViT(
        patch_size=patch_size, embed_dim=32, depth=1, num_heads=1, in_chans=in_chans,
        decoder_embed_dim=96, decoder_depth=14, decoder_num_heads=8,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def mae_vit_15(patch_size, in_chans, **kwargs):
    model = FViT(
        patch_size=patch_size, embed_dim=32, depth=1, num_heads=1, in_chans=in_chans,
        decoder_embed_dim=120, decoder_depth=12, decoder_num_heads=8,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def mae_vit_16(patch_size, in_chans, **kwargs):
    model = FViT(
        patch_size=patch_size, embed_dim=32, depth=1, num_heads=1, in_chans=in_chans,
        decoder_embed_dim=96, decoder_depth=16, decoder_num_heads=8, norm_pix_loss=True,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model