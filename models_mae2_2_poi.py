...
from util.data_process import MinMaxNormalization
...
class MaskedAutoencoderViT(nn.Module):
    ...
    """
    def __init__(self, img_size=8, patch_size=1, in_chans=1, sample_index=None, margin=20,
                 embed_dim=1024, depth=24, num_heads=16, fraction=20, T_len=5, type='softmax',
                 decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False):
        super().__init__()
        self.poi = np.load("data/POI.npy")
        self.poi = torch.from_numpy(self.poi).cuda().float()
        self.poi_embed = nn.Linear(14, 1, bias=True)

        # self.mmn = MinMaxNormalization()
        # poi = self.poi.reshape(8 * 8 * 14)
        # self.mmn.fit(poi)
        # self.poi = poi.reshape(8, 8, 14)
        ...
        self.mask_token = nn.Parameter(torch.zeros(1, 1, decoder_embed_dim))  # (N, L ,D)
        self.time_mask_token = nn.Parameter(torch.zeros(1, 1, 1, decoder_embed_dim)) # (N,L,T,D)
        ...
        self.time_decoder_embed = nn.Parameter(torch.zeros(1, self.T_len + 1, decoder_embed_dim))
    ...
    def forward_encoder(self, hx, x):
        ...
        # TimeSformer
        tokens = x[:, :1, :]
        x = rearrange(x[:, 1:, :], 'b (l t) d -> b l t d', b=B, l=L)
        ...
        # time_x = x[:, :, start: start + self.plen + self.tlen, :]
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        s_x = self.decoder_norm(x)

        # predictor projection
        x = self.decoder_pred(s_x)
    ...
    def forward_poiloss(self, latent):
        poi = self.poi
        poi = self.poi_embed(poi).squeeze(-1)
        loss = loss_c(latent, poi, self.margin, self.type)

        return loss

    # Time Loss
    def forward_tloss(self, s_latent, time_latent):
        time_latent1 = torch.mean(time_latent, dim=2)
        loss = (s_latent - time_latent1) ** 2
        loss = loss.mean()
    ...
    def forward(self, *args):
        ...
        N, C, H, W = X.shape
        # poi = self.poi_embed(self.poi).unsqueeze(0).repeat(N, 1, 1, 1).permute(0, 3, 1, 2)
        ...
        poi_loss = self.forward_poiloss(s_pred)
        ...
        return loss, all_loss, poi_loss, s_loss, t_loss, s_loss, t_loss, pred, s_pred
